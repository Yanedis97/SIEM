...
def check_snort_alert(log, es):
    ...
    # Comprobar si log es una lista de registros
    if isinstance(log, list):
        for entry in log:
            process_snort_log(entry, es)
    else:
        # Si log no es una lista, procesarlo directamente
...
def process_snort_log(log_entry, es):
    ...
    if log_entry.get('_source', {}).get('type') == 'snort':
        ...
        # Verificar si '@timestamp' está presente y tiene el formato adecuado
        ...
        if timestamp_str:
            try:
                ...
                timestamp = datetime.now()  # Usar la hora actual como alternativa
        else:
            print(">>> '@timestamp' no encontrado, usando hora actual.")
            timestamp = datetime.now()  # Usar la hora actual si no está disponible
        ...
        if not is_alert_active(alert_id):
            ...
            log_id = log_entry.get('_id', 'ID desconocido')  # O el campo que identifica el log
            activate_alert(alert_id, message, es, context=context)
    else:
        print(">>>>Error en Snort: no es un tipo de log de Snort.")
